...
try:
    while 1:
        ...
        # 전체적인 구조
        ### 1. 정시에 작동되는 프로세스 실행
        ##### 1. 생장조건 갱신 << ++ 회원 테이블 생성 해야함, ++ a
        ##### 2. 보고서 작성 << ++ 보고서용 데이터 추출, ++ 그에 맞게 이미지 생성, ++ docx에 붙이기, ++ 송신
        ### 2. RPi로 부터 메시지 수신
        ##### 1. 메시지 수신, 변환, 가공 << ++ 정시 작동을 위해 특정 시간에는 빠져 나오도록? 하거나 추가 설정이 필요함
        ##### 2. 수신 데이터 분기점 시작 
        ####### 1. 정시보고(센싱), 사용자 요청 갱신(센싱+보고서), 식물변경, 보고서요청
        ######### 1. 정시보고(센싱) : 데이터 저장
        ######### 2. 수시보고(센싱+보고서) : 데이터 저장 - 보고서 데이터 추출 - 보고서 생성 - 수신
        ######### 3. 식물변경 : 생장정보에서 바꿀 데이터 추출 - 송신
        ######### 4. 보고서 요청 : 송신으로 바로 연결 
